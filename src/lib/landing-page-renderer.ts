...
import type { BrandImage, Brand } from '@/types'
...
export type RenderLandingOptions = {
  ...
  supabaseUrl: string
  // Optional palette/font overrides — when absent, falls back to brand.* values.
  primary?: string
  secondary?: string
  accent?: string
  font?: string
...
export function renderLandingHtml(opts: RenderLandingOptions): string {
  const { brand, brief, brandImages, supabaseUrl } = opts

  const getUrl = (path: string) =>
    `${supabaseUrl}/storage/v1/object/public/brand-images/${path}`

  const rawImages = brandImages
  ...
  const primary = opts.primary || brand.primary_color || '#000000'
  const secondary = opts.secondary || brand.secondary_color || primary
  const accent = opts.accent || brand.accent_color || secondary
  ...
  const fontFamily = opts.font
    || (brand.font_heading as { family?: string } | null | undefined)?.family
    || brand.font_primary?.split('|')[0]
    || 'system-ui'
  const fontWeight = (brand.font_heading as { weight?: string } | null | undefined)?.weight || '700'
  const fontTransform = opts.transform
    || (brand.font_heading as { transform?: string } | null | undefined)?.transform
    || 'none'
  ...
  type Benefit = { headline: string; body: string }
  type FaqEntry = { question: string; answer: string }

  const benefitsHtml = (brief.benefits || []).slice(0, 4).map((b: Benefit) => `
  ...
  const faqHtml = (brief.faq || []).map((f: FaqEntry) => `
...
.heading { font-family: '${fontFamily}', system-ui, sans-serif !important; font-weight: ${fontWeight} !important; text-transform: ${fontTransform} !important; }
  ...
  const microHtml = microBenefits.map((b: Benefit) => `
  ...
  const proofHtml = (brief.benefits || []).slice(0, 4).map((b: Benefit) => `
  ...
  const ingredientCardsHtml = (brief.benefits || []).slice(0, 3).map((b: Benefit, i: number) => {
  ...
  const pressBenefitBar = (brief.benefits || []).slice(0, 5).map((b: Benefit) => `
  ...
  const guaranteeHtml = (brief.benefits || []).slice(0, 3).map((b: Benefit) => `
