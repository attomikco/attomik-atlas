...
export function GalleryBlock({ block }: { block: Block }) {
  ...
  // Phase 3 renders 6 placeholder tiles regardless of items content — real
  // image refs land in Phase 6 with the Assets panel.
  const count = (d.items?.length ?? 6) || 6
  return (
    <div style={{ padding: `${spacing[16] + 8}px ${spacing[16]}px`, background: colors.cream }}>
